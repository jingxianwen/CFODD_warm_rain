                ...
                if (sgl_idx .eqv. .true.)  then
                  ...
                  !count for CFODD
                    if (inaerbin .ge.1 .and. inaerbin .le. num_naerbnd .and. &
                        irebin .ge.1 .and. irebin .le. nmax_reff .and. &
                      ...
                      cnt_cfodd(inaerbin,irebin,icotbin,izebin)=cnt_cfodd(inaerbin,irebin,icotbin,izebin)+1
                    end if
                  end do !k
                 end if !sgl_idx
               end if !cld_idx
...
!----write out counter for CFODD
  do iaer=1,num_naerbnd
   do i=1,nmax_reff
    open (50,file=trim(out_path)//'cfodd_'//trim(naerbnd_str(iaer))//"_"//trim(rerng(i))//".txt", &
         form='formatted')
     do j=1,nmax_tauc
       write (50,*) cnt_cfodd(iaer,i,j,:)
     end do
    close (50)
   end do
  end do
